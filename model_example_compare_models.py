...
class GeometryBoundaryConditionAndWaveFunction:
    def data_to_export(self):
        """Save fracture quantities to text files for plotting and analysis.
        ...
        traction_t_Pa = self.equation_system.evaluate(nd_vec_to_tangential @ traction)
        traction_n_Pa = self.equation_system.evaluate(nd_vec_to_normal @ traction)
        ...
        traction = self.evaluate_and_scale([sd_frac], "contact_traction", "-").reshape(
            (self.nd, -1), order="F"
        )
        ...
        displacement_jump_file_n = os.path.join(results_dir, "displacement_jump_n.txt")
        displacement_jump_file_t = os.path.join(results_dir, "displacement_jump_t.txt")

        traction_file_n_Pa = os.path.join(results_dir, "traction_n.txt")
        traction_file_t_Pa = os.path.join(results_dir, "traction_t.txt")
...
class SL(GeometryBoundaryConditionAndWaveFunction, SpringTypeLinear):
    """Model setup for the self convergence analysis: Linear spring type.
...
class SBB(GeometryBoundaryConditionAndWaveFunction, SpringTypeBartonBandis):
    """Model setup for the self convergence analysis: Barton-Bandis spring type.
