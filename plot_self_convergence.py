...
def plot_all(data):
    ...
    for title, key, cbar_label, use_contours in fields:
        fig, subfigs = plt.subplots(
            1, 2, figsize=(14, 6), sharey=True, layout="constrained"
        )
        ...
        cbar = fig.colorbar(ims[-1], ax=subfigs, location="right", shrink=0.9, pad=0.0)
...
def plot_line_last_timestep(data, case="CL"):
    ...
    # ========================================================
    # SAFE: use opening as authoritative mesh size
    # ========================================================
    n = len(opening)
    ...
    ax1.set_ylim(
        -np.nanmax(opening) * 0.02, np.nanmax(opening) * 1.1
    ) 
    ax1_right = ax1.twinx()
    ax1_right.set_ylim(-np.nanmax(slip) * 0.018, np.nanmax(slip) * 1.4)
    ...
    ax2.set_ylim(
        -np.nanmax(tau_t) * 1.5, np.nanmax(tau_t) * 3.0
    ) 
